...
class DataSet:
    ...
    def __init__(self, txt_files, thread_count, batch_size, numcep, numcontext):
        ...
        self._start_idx = 0
        self.data = np.loadtxt(self._txt_files[0])
    ...
    def next_batch(self, batch_size=None):
        ...
        current_data = self.data[self._start_idx:end_idx, :]
        # get features and normalize it
        source = current_data[:, 0:242]
        source = stats.zscore(source)
        source[np.isnan(source)]=0
        # get labels and transform it to [batch_size, num_step, number_classes]
        target = current_data[:, 249:]
        # -1 for "0"????
        s = ['0', '406516', '406517', '404516','404517', '406520'
            , '404520', '406505','404505', '406519','404519', '406511'
            , '404511','406508', '404508', '408512', '407521', '405506']
        indice, value = [], []
        for index,i in enumerate(target[:,0]):
            p = str(int(i))
            value.append(s.index(p))
        ...
        self._start_idx += batch_size
        # Verify that the start_idx is not larger than total available sample size
        if self._start_idx >= self.data.shape[0]:
            self._start_idx = 0
        s = np.reshape(source, (50,100,242))
        # Pad input to max_time_step of this batch
        source, source_lengths = pad_sequences(s)

        return s, source_lengths, value.reshape([50,100])
